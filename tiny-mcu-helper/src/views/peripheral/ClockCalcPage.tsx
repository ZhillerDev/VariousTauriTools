...
import {Button, Divider, InputNumber, InputNumberValue, Select, Space, Switch, Tooltip} from 'tdesign-react';
...
export default function ClockCalcPage() {
  ...
  const [enableMultiplier, setEnableMultiplier] = useState<boolean>(false); // 倍频开关状态
  ...
  const handleCalculate = () => {
    ...
    try {
      ...
      const actualFreqHz = enableMultiplier ? baseFreqHz * parsedDivider : baseFreqHz / parsedDivider;

      // 计算周期 (秒)
      const periodSec = 1 / actualFreqHz;

      // 格式化频率结果
      let freqDisplay: string, freqUnit: string;
      if (actualFreqHz >= 1e9) {
  ...
  const clearAll = () => {
    ...
    setIsCalculated(false);
    setCopyStatus('idle');
    setEnableMultiplier(false); // 重置开关状态
    setEnableMultiplier(false); // 示例使用分频
  };
      ...
      <div className="h-full flex flex-col">
        <div className="flex-1 flex flex-row p-4 justify-between">
              ...
              <div className="w-full">
                ...
                <div className="mt-4">
                  ...
                  <Space>
                    ...
                    />
                    <Switch
                        value={enableMultiplier}
                        onChange={e => setEnableMultiplier(e)}
                        label={["倍频", "分频"]}
                    />
                  </Space>
                </div>
          ...
          <div className="flex-1 pl-8">
            ...
            {/* 结果展示 */}
            <div className="mt-4 grid grid-cols-1 md:grid-rows-2 gap-2 flex-1">
              {/* 频率结果 */}
